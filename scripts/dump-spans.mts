...
const front = edges.filter((e) => e.lenPt > 1e-6 && sideOfPerimeterEdge(e.p1, e.p2, outline) === "front");
let lo = Infinity, hi = -Infinity;
for (const e of front) { lo = Math.min(lo, e.p1.x, e.p2.x); hi = Math.max(hi, e.p1.x, e.p2.x); }
for (const e of front) {
  const u0 = (Math.min(e.p1.x, e.p2.x) - lo) / (hi - lo);
  const u1 = (Math.max(e.p1.x, e.p2.x) - lo) / (hi - lo);
