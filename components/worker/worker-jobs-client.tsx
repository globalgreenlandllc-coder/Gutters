      ...
      <div className="inline-flex rounded-xl bg-zinc-100 p-1">
        {tabs.map((t) => (
          ...
          >
            {t.label}
            {t.count > 0 && (
              <span className={cn("ml-1.5 rounded-full px-1.5 py-0.5 text-[11px]", filter === t.key ? "bg-accent-50 text-accent-700" : "bg-zinc-200 text-zinc-500")}>
                {t.count}
              </span>
            )}
          </button>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {jobs.map((j) => (
            <Link
              ...
              className="group surface flex flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white transition-shadow hover:shadow-card"
            >
              <div className="flex items-start justify-between gap-2 px-4 pt-4">
                ...
                <ChevronRight className="mt-1 h-4 w-4 shrink-0 text-zinc-300 group-hover:text-zinc-500" />
              </div>
